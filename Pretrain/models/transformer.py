...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center, fps_idx = misc.fps(xyz, self.num_group)  # B G 3
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=2., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., finetune=False):
        super().__init__()
...
class TransformerDecoder(nn.Module):
    def __init__(self, embed_dim=384, depth=4, num_heads=6, mlp_ratio=2., qkv_bias=False, qk_scale=None,
    ...
    def forward(self, x, pos, return_token_num):
        for _, block in enumerate(self.blocks):
            x = block(x + pos)
        x_vis = self.head(self.norm(x[:, :-return_token_num]))
        x_mask = self.head(self.norm(x[:, -return_token_num:]))  # only return the mask tokens predict pixel
        return x_mask, x_vis
...
# zeroshot model
@MODELS.register_module()
class PointTransformerZeroShot(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        self.type = config.type
        ...
        if self.type == 'full':
            self.text_proj = nn.Linear(self.trans_dim, TextEncoder(config).embed_dim)
            self.img_proj = nn.Linear(self.trans_dim, ImageEncoder(config).output_dim)
        elif self.type == 'text':
            self.text_proj = nn.Linear(self.trans_dim, TextEncoder(config).embed_dim)
    ...
    def forward(self, pts):
        ...
        x = self.norm(x)
        if self.type == 'text':
            feature = self.text_proj(x[:, 2])
        elif self.type == 'image':
            feature = self.img_proj(x[:, 1])
        else:
            text_feature = self.text_proj(x[:, 2])
            img_feature = self.img_proj(x[:, 1])
            feature = self.ratio * text_feature + img_feature
        return feature


# svm model
@MODELS.register_module()
class PointTransformerSVM(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
    ...
    def forward(self, pts):
        ...
        x = self.blocks(x, pos)
        x = self.norm(x)
        feature = torch.cat([x[:, :3].max(1)[0], x[:, 3:].max(1)[0] + x[:, 3:].mean(1)], dim=-1)
        return feature
