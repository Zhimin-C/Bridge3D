...
class ReCon(nn.Module):
    def __init__(self, config):
        ...
        self.rec_head_2d = nn.Conv1d(self.trans_dim, 1024, 1)
        self.rec_head_txt = nn.Conv1d(self.trans_dim, 512, 1)
    ...
    def forward(self, pts, data, **kwargs):
        ...
        paired_imgpose, img, pred_mask, pred_sem, caption_feat, img_scene_feat, img_obj_feat= data[0].cuda(), data[1].cuda(), data[2].cuda(), data[3].cuda(), data[4].cuda(), data[5].cuda(), data[6].cuda()
        ...
        pairing_images[:, 1], pairing_images[:, 1] = pairing_images[:, 2], pairing_images[:, 1]
        m = tuple(pairing_images.cpu().T.long())

        torch.index_select(pred_sem, 0, pairing_visfps_idx.cuda())
        pairing_sem_label = pred_sem[pairing_visfps_idx.long()]
        pairing_sem_label_ori = pairing_sem_label.clone()
        pairing_sem_label[pairing_sem_label==255] = 0
        pairing_sem_label = pairing_sem_label.unsqueeze(-1).repeat(1, 512)

        masked_pairing_sem_label = pred_sem[pairing_maskfps_idx.long()]
        mask_fg_num = (masked_pairing_sem_label != 255).sum()

        drop_num = int(self.drop_ratio * center.shape[1]) * center.shape[0]

        bg_indx = (masked_pairing_sem_label==255).nonzero()
        fg_indx = (masked_pairing_sem_label!=255).nonzero()
        ...
        txt_mask = pairing_sem_label_ori!=255
        ...
        pred_obj_2D_feats = self.rec_head_2d(x_rec_vis.reshape(-1, C).unsqueeze(-1)).view(-1, img_obj_feat.shape[1])
        pred_obj_txt_feats = self.rec_head_txt(x_rec_vis.reshape(-1, C).unsqueeze(-1)).view(-1,scannete_txt_features.shape[1])
        ...
        losses['obj_img'] = self.csc_loss(k, q).mean()
